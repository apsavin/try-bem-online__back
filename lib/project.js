...
function getFullPath (projectId, relativePath) {
    return path.join(config.tmpPath, projectId, relativePath == null ? '' : String(relativePath));
}
...
var bemPath = path.join(config.examplePath, 'node_modules/bem/bin/bem make'),
...
function bemMake (projectId, callback, method) {
    ...
    exec(bemPath + ' -r ' + getFullPath(projectId) + method, function (err) {
